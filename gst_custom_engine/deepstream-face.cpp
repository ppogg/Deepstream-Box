...
std::string getCurrentTimeFilename(const char *file_format)
{
    ...
    return "Face" + oss.str() + file_format_name;
}
...
static void set_custom_bbox(NvDsFrameMeta *frame_meta, NvDsObjectMeta *obj_meta, FaceData *Face_data)
{
    guint border_width = 6;
    guint font_size = 18;
    guint x_offset = MIN((gfloat)Face_data->video_width - 1, 
        (guint)MAX(0, (gint)(obj_meta->rect_params.left - (border_width / 2))));
    guint y_offset = MIN((gfloat)Face_data->video_height - 1, 
        (guint)MAX(0, (gint)(obj_meta->rect_params.top - (font_size * 2) + 1)));
...
static void parse_Face_from_meta(NvDsFrameMeta *frame_meta, NvDsObjectMeta *obj_meta, FaceData *Face_data)
{
    ...
    gfloat video_width = (gfloat)Face_data->video_width;
    gfloat video_height = (gfloat)Face_data->video_height;
    ...
    for (guint i = 0; i < num_joints; ++i)
    {
        ...
        circle_params->circle_color.alpha = 0.0;
...
static GstPadProbeReturn infer_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
    ...
    FaceData *Face_data = (FaceData *)user_data;
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next)
    {
        ...
        for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next)
        {
            ...
            parse_Face_from_meta(frame_meta, obj_meta, Face_data);
            set_custom_bbox(frame_meta, obj_meta, Face_data);
        }
    }
...
{
    std::string config_file = "Face.txt";
    if (argc > 1) {
        config_file = argv[1];
    }
    ...
    std::string VIDEO_PATH = config.get_string("source", "uri", "../bodyFace.mp4");
    std::string CONFIG_FILE_PATH = config.get_string("inference", "config-file-path", "../config_infer_cu.txt");
    
    FaceData Face_data;
    Face_data.video_width = SET_WIDTH;
    Face_data.video_height = SET_HEIGHT;
    ...
    if (!infer_src_pad)
    {
        ...
        gst_pad_add_probe(infer_src_pad, GST_PAD_PROBE_TYPE_BUFFER, infer_src_pad_buffer_probe, &Face_data, NULL);
