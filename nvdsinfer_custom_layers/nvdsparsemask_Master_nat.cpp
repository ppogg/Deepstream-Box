...
extern "C" bool NvDsInferParseMasterSeg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
...
static std::vector<NvDsInferInstanceMaskInfo> decodeTensorMasterSeg(const float *output, size_t outputSize,
...
static bool
NvDsInferParseCustomMasterSeg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
                            NvDsInferNetworkInfo const &networkInfo,
                            NvDsInferParseDetectionParams const &detectionParams,
                            std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  ...
  std::vector<NvDsInferInstanceMaskInfo> objects = decodeTensorMasterSeg((const float *)(output.buffer), outputSize,
...
extern "C" bool
NvDsInferParseMasterSeg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo, NvDsInferNetworkInfo const &networkInfo,
                      NvDsInferParseDetectionParams const &detectionParams, std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  return NvDsInferParseCustomMasterSeg(outputLayersInfo, networkInfo, detectionParams, objectList);
}

CHECK_CUSTOM_INSTANCE_MASK_PARSE_FUNC_PROTOTYPE(NvDsInferParseMasterSeg);
