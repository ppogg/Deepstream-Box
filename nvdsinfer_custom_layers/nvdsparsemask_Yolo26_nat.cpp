...
extern "C" bool NvDsInferParseYolo26Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
...
static std::vector<NvDsInferInstanceMaskInfo> decodeTensorYolo26Seg(const float *output, size_t outputSize,
...
static bool
NvDsInferParseCustomYolo26Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
                            NvDsInferNetworkInfo const &networkInfo,
                            NvDsInferParseDetectionParams const &detectionParams,
                            std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  ...
  std::vector<NvDsInferInstanceMaskInfo> objects = decodeTensorYolo26Seg((const float *)(output.buffer), outputSize,
...
extern "C" bool
NvDsInferParseYolo26Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo, NvDsInferNetworkInfo const &networkInfo,
                      NvDsInferParseDetectionParams const &detectionParams, std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  return NvDsInferParseCustomYolo26Seg(outputLayersInfo, networkInfo, detectionParams, objectList);
}
