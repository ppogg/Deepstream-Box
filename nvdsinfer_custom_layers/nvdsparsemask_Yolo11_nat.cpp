...
extern "C" bool NvDsInferParseYolo11Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
...
static std::vector<NvDsInferInstanceMaskInfo> decodeTensorYolo11Seg(const float *output, size_t outputSize,
...
static bool
NvDsInferParseCustomYolo11Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
                            NvDsInferNetworkInfo const &networkInfo,
                            NvDsInferParseDetectionParams const &detectionParams,
                            std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  ...
  std::vector<NvDsInferInstanceMaskInfo> objects = decodeTensorYolo11Seg((const float *)(output.buffer), outputSize,
...
extern "C" bool
NvDsInferParseYolo11Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo, NvDsInferNetworkInfo const &networkInfo,
                      NvDsInferParseDetectionParams const &detectionParams, std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  return NvDsInferParseCustomYolo11Seg(outputLayersInfo, networkInfo, detectionParams, objectList);
}
