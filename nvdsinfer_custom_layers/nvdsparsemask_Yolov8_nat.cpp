...
extern "C" bool NvDsInferParseYolov8Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
...
static std::vector<NvDsInferInstanceMaskInfo> decodeTensorYolov8Seg(const float *output, size_t outputSize,
...
static bool
NvDsInferParseCustomYolov8Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
                            NvDsInferNetworkInfo const &networkInfo,
                            NvDsInferParseDetectionParams const &detectionParams,
                            std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  ...
  std::vector<NvDsInferInstanceMaskInfo> objects = decodeTensorYolov8Seg((const float *)(output.buffer), outputSize,
...
extern "C" bool
NvDsInferParseYolov8Seg(std::vector<NvDsInferLayerInfo> const &outputLayersInfo, NvDsInferNetworkInfo const &networkInfo,
                      NvDsInferParseDetectionParams const &detectionParams, std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  return NvDsInferParseCustomYolov8Seg(outputLayersInfo, networkInfo, detectionParams, objectList);
}

CHECK_CUSTOM_INSTANCE_MASK_PARSE_FUNC_PROTOTYPE(NvDsInferParseYolov8Seg);
