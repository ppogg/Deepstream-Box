...
```
python export_ultralytics.py --weight yolo26n-cls.pt --task 4 --dynamic
```
c. export pose onnx.
...
```
python export_ultralytics.py --weight yolo26n-seg.pt --task 2
```
...
```
python ptq.py --onnx yolo26n.onnx --calib_data images/ --batch_size 32
```
b. export cls int8 engine.

```
python ptq.py --onnx yolo26n-cls.onnx --calib_data images/ --batch_size 32
```
c. export pose int8 engine.

```
python ptq.py --onnx yolo26n-pose.onnx --calib_data images/ --batch_size 32
```
d. export seg int8 engine.

```
python ptq.py --onnx yolo26n-seg.onnx
```
**NOTE**: Segment task only suppurt `batch=1`.
##
...
[primary-gie]
...
config-file=yolo26/config_infer_dbox_yolo26.txt
```
Then adjust `config_infer_dbox_yolo26.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolo26/yolo26n-int8.engine
network-mode=1
network-type=0
cluster-mode=2
parse-bbox-func-name=NvDsInferParseCustomyolo26_cuda
```
...
[primary-gie]
...
config-file=yolo26/config_infer_mask_yolo26.txt
```
Then adjust `config_infer_mask_yolo26.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolo26/yolo26n-seg_int8.engine
network-mode=2
network-type=3
cluster-mode=4
parse-bbox-func-name=NvDsInferParseyolo26Seg
...
...
```
**NOTE 2**: **Pose Task** and **Cls Task** Similar to **Det Task**.
