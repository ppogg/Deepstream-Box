...
#### 1.1 Download the YOLOv7 repo and install the requirements
...
Copy `export_yolov7.py`.

```
cp Deepstream-Box/scripts/export_yolov7.py yolov7/
```

#### 1.4 Download the model

Download the `.pt` from [YOLOv7](https://github.com/WongKinYiu/yolov7/releases) releases, like
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolov7t.pt |The original .pt model. |
...
```
python export_yolov7.py --weight yolov7t.pt --task 0 --dynamic
```
##
...
```
python ptq.py --onnx yolov7t.onnx --calib_data images/ --batch_size 32
```
##
...
[primary-gie]
...
config-file=yolov7/config_infer_dbox_yolov7.txt
```
Then adjust `config_infer_dbox_yolov7.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov7/yolov7t-int8.engine
network-mode=1
network-type=0
cluster-mode=2
