...
#### 1.1 Download the YOLOv13 repo and install the requirements
...
Download the `.pt` from [YOLOv13](https://github.com/iMoonLab/yolov13/releases) releases, like
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolov13n.pt |The original .pt model. |
...
```
python export_ultralytics.py --weight yolov13n.pt --task 0 --dynamic
```
##
...
| **check_height** | int |640 |Fixed input width (if ONNX has dynamic shape) (default: 640). |
...
```
python ptq.py --onnx yolov13n.onnx --calib_data images/ --batch_size 32
```
##
...
[primary-gie]
...
config-file=yolo13/config_infer_dbox_yolo13.txt
```
Then adjust `config_infer_dbox_yolo13.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolo13/yolo13n-int8.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=80
parse-bbox-func-name=NvDsInferParseCustomYolo13_cuda
```
##
