...
#### 1.1 Download the YOLOv5 repo and install the requirements
...
```
cp Deepstream-Box/scripts/export_yolov5.py yolov5/
```

#### 1.4 Download the model

Download the `.pt` from [YOLOv5](https://github.com/ultralytics/yolov5/releases) releases, like
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolov5s.pt |The original .pt model. |
...
```
python export_yolov5.py --weight yolov5s.pt --task 0 --dynamic
```
##
...
```
python ptq.py --onnx yolov5s.onnx --calib_data images/ --batch_size 32
```
##
...
[primary-gie]
...
config-file=yolov5/config_infer_dbox_yolov5.txt
```
Then adjust `config_infer_dbox_yolov5.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov5/yolov5s-int8.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=80
parse-bbox-func-name=NvDsInferParseCustomYolov5_cuda
```
##
