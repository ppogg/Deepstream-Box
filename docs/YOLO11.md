...
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolo11n-cls.pt
```
c. Keypoint task 👇
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolo11n-pose.pt
```
d. Segment task 👇
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolo11n-seg.pt
```
##
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolo11n.pt |The original .pt model. |
...
```
python export_ultralytics.py --weight yolo11n.pt --task 0 --dynamic
```
b. export cls onnx.

```
python export_ultralytics.py --weight yolo11n-cls.pt --task 4 --dynamic
```
c. export pose onnx.

```
python export_ultralytics.py --weight yolo11n-pose.pt --task 1 --dynamic
```
d. export seg onnx.

```
python export_ultralytics.py --weight yolo11n-seg.pt --task 2
```
...
```
python ptq.py --onnx yolo11n.onnx --calib_data images/ --batch_size 32
```
b. export cls int8 engine.

```
python ptq.py --onnx yolo11n-cls.onnx --calib_data images/ --batch_size 32
```
c. export pose int8 engine.

```
python ptq.py --onnx yolo11n-pose.onnx --calib_data images/ --batch_size 32
```
d. export seg int8 engine.

```
python ptq.py --onnx yolo11n-seg.onnx
```
...
[primary-gie]
...
config-file=yolo11/config_infer_dbox_yolo11.txt
```
Then adjust `config_infer_dbox_yolo11.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolo11/yolo11n-int8.engine
network-mode=1
network-type=0
cluster-mode=2
parse-bbox-func-name=NvDsInferParseCustomYolo11_cuda
```
...
[primary-gie]
...
config-file=yolo11/config_infer_mask_yolo11.txt
```
Then adjust `config_infer_mask_yolo11.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolo11/yolo11n-seg_fp16.engine
network-mode=2
network-type=3
cluster-mode=4
parse-bbox-func-name=NvDsInferParseYolo11Seg
...
#### 6.4 For pose task.
```
deepstream-pose -c deepstream_app_config_pose.txt
```
