...
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolov8n-cls.pt
```
c. Keypoint task 👇
```
...
d. Segment task 👇
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolov8n-seg.pt
```
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolov8n.pt |The original .pt model. |
...
```
python export_ultralytics.py --weight yolov8n.pt --task 0 --dynamic
```
b. export cls onnx.

```
python export_ultralytics.py --weight yolov8n-cls.pt --task 4 --dynamic
```
c. export pose onnx.

```
python export_ultralytics.py --weight yolov8n-pose.pt --task 1 --dynamic
```
d. export seg onnx.

```
python export_ultralytics.py --weight yolov8n-seg.pt --task 2
```
...
```
python ptq.py --onnx yolov8n.onnx --calib_data images/ --batch_size 32
```
b. export cls int8 engine.

```
python ptq.py --onnx yolov8n-cls.onnx --calib_data images/ --batch_size 32
```
c. export pose int8 engine.

```
python ptq.py --onnx yolov8n-pose.onnx --calib_data images/ --batch_size 32
```
d. export seg int8 engine.

```
python ptq.py --onnx yolov8n-seg.onnx
```
...
[primary-gie]
...
config-file=yolov8/config_infer_dbox_yolov8.txt
```
Then adjust `config_infer_dbox_yolov8.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov8/yolov8n-int8.engine
network-mode=1
network-type=0
cluster-mode=2
...
[primary-gie]
...
config-file=yolov8/config_infer_mask_yolov8.txt
```
Then adjust `config_infer_mask_yolov8.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov8/yolov8n-seg_fp16.engine
network-mode=2
network-type=3
cluster-mode=4
parse-bbox-func-name=NvDsInferParseYolov8Seg
