...
#### 1.1 Download the YOLOv6 repo and install the requirements
...
Copy `export_yolo6.py`.

```
cp Deepstream-Box/scripts/export_yolov6.py YOLOv6/
```

#### 1.4 Download the model

Download the `.pt` from [YOLOv6](https://github.com/meituan/YOLOv6/releases) releases, like
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolov6n.pt |The original .pt model. |
...
```
python export_yolov6.py --weight yolov6n.pt --task 0 --dynamic
```
##
...
```
python ptq.py --onnx yolov6n.onnx --calib_data images/ --batch_size 32
```
##
...
[primary-gie]
...
config-file=yolov6/config_infer_dbox_yolov6.txt
```
Then adjust `config_infer_dbox_yolov6.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov6/yolov6n-int8.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=80
parse-bbox-func-name=NvDsInferParseCustomYolov6_cuda
```
##
