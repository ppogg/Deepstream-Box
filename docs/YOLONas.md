...
**a. Detection task** 👇
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolo_nas_s.pt
```
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolo_nas_s.pt |The original .pt model. |
| **size** | int |640 |Network input size. |
| **task** | int |3 |Task type (0-det/1-kpt/2-seg/3-nas/4-cls). |
...
```
python export_ultralytics.py --weight yolo_nas_s.pt --task 3 --dynamic
```
##
...
```
python ptq.py --onnx yolo_nas_s.onnx --calib_data images/ --batch_size 32
```
##
...
[primary-gie]
...
config-file=yolonas/config_infer_dbox_yolonas.txt
```
Then adjust `config_infer_dbox_yolonas.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolonas/yolo_nas_s-int8.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=80
parse-bbox-func-name=NvDsInferParseCustomYolonas_cuda
```
##
