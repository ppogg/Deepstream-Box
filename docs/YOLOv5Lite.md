...
#### 1.1 Download the YOLOv5-Lite repo and install the requirements
...
Copy `export_yolov5.py`.

```
cp Deepstream-Box/scripts/export_yolov5.py YOLOv5-Lite/
```

#### 1.4 Download the model

Download the `.pt` from [YOLOv5-Lite](https://drive.google.com/file/d/1oftzqOREGqDCerf7DtD5BZp9YWELlkMe/view?usp=sharing) releases, like

**a. Detection task** 👇
```
wget https://drive.google.com/file/d/1oftzqOREGqDCerf7DtD5BZp9YWELlkMe/view?usp=sharing
```
...
| :---: | :---: | :---: | :---: |
| **weight** | str | v5lite-g.pt |The original .pt model. |
...
```
python export_yolov5.py --weight v5lite-g.pt --task 0 --dynamic
```
##
...
```
python ptq.py --onnx v5lite-g.onnx --calib_data images/ --batch_size 32
```
##
...
[primary-gie]
...
config-file=yolov5lite/config_infer_dbox_v5Lite.txt
```
Then adjust `config_infer_dbox_v5Lite.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov5lite/v5lite-g-int8.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=80
parse-bbox-func-name=NvDsInferParseCustomv5Lite_cuda
```
##
