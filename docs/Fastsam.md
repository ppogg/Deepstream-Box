...
**a. Detection task** 👇
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/FastSAM-s.pt
```
...
| :---: | :---: | :---: | :---: |
| **weight** | str | FastSAM-s.pt |The original .pt model. |
| **size** | int |640 |Network input size. |
| **task** | int |2 |Task type (0-det/1-kpt/2-seg/3-nas/4-cls). |
...
```
python export_ultralytics.py --weight FastSAM-s.pt --task 2 --dynamic
```
##
...
```
python ptq.py --onnx FastSAM-s.onnx --calib_data images/ --batch_size 32
```
##
...
[primary-gie]
...
config-file=fastsam/config_infer_mask_fastsam.txt
```
Then adjust `config_infer_mask_fastsam.txt`.
```
[property]
...
model-engine-file=../../model_zoo/fastsam/FastSAM-s_fp16.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=100
parse-bbox-func-name=NvDsInferParseFastsamSeg
```
##
