...
#### 1.1 Download the YOLOv12 repo and install the requirements
...
```
cp Deepstream-Box/scripts/export_ultralytics.py yolov12/ultralytics/
```

#### 1.4 Download the model

Download the `.pt` from [YOLO12](https://github.com/sunsmarterjie/yolov12/releases) releases, like
...
```
wget https://github.com/sunsmarterjie/yolov12/releases/download/cls/yolov12n-cls.pt
```
c. Segment task 👇
```
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolov12n.pt |The original .pt model. |
...
```
python export_ultralytics.py --weight yolov12n.pt --task 0 --dynamic
```
b. export cls onnx.

```
python export_ultralytics.py --weight yolov12n-cls.pt --task 4 --dynamic
```
c. export seg onnx.

```
python export_ultralytics.py --weight yolov12n-seg.pt --task 2
```
...
```
python ptq.py --onnx yolov12n.onnx --calib_data images/ --batch_size 32
```
b. export cls int8 engine.

```
python ptq.py --onnx yolov12n-cls.onnx --calib_data images/ --batch_size 32
```
c. export seg int8 engine.

```
python ptq.py --onnx yolov12n-seg.onnx
```
...
[primary-gie]
...
config-file=yolo12/config_infer_dbox_yolo12.txt
```
Then adjust `config_infer_dbox_yolo12.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolo12/yolo12n-int8.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=80
parse-bbox-func-name=NvDsInferParseCustomYolo12_cuda
```
...
[primary-gie]
...
config-file=yolo12/config_infer_mask_yolo12.txt
```
Then adjust `config_infer_mask_yolo12.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolo12/yolo12n-seg_fp16.engine
network-mode=2
network-type=3
cluster-mode=4
parse-bbox-func-name=NvDsInferParseYolo12Seg
