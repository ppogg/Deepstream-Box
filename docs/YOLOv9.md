...
**a. Detection task** 👇
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolov9t.pt
```
b. Segment task 👇
```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolov9c-seg.pt
```
##
...
| :---: | :---: | :---: | :---: |
| **weight** | str | yolov9t.pt |The original .pt model. |
...
```
python export_ultralytics.py --weight yolov9t.pt --task 0 --dynamic
```
b. export seg onnx.

```
python export_ultralytics.py --weight yolov9c-seg.pt --task 2
```
...
```
python ptq.py --onnx yolov9t.onnx --calib_data images/ --batch_size 32
```
b. export seg int8 engine.

```
python ptq.py --onnx yolov9c-seg.onnx
```
...
[primary-gie]
...
config-file=yolov9/config_infer_dbox_yolov9.txt
```
Then adjust `config_infer_dbox_yolov9.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov9/yolov9t-int8.engine
network-mode=1
network-type=0
cluster-mode=2
num-detected-classes=80
parse-bbox-func-name=NvDsInferParseCustomYolov9_cuda
```
...
[primary-gie]
...
config-file=yolov9/config_infer_mask_yolov9.txt
```
Then adjust `config_infer_mask_yolov9.txt`.
```
[property]
...
model-engine-file=../../model_zoo/yolov9/yolov9c-seg_fp16.engine
network-mode=2
network-type=3
cluster-mode=4
parse-bbox-func-name=NvDsInferParseYolov9Seg
